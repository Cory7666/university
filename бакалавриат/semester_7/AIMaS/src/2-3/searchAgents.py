...
class CornersProblem(search.SearchProblem):
    ...
    def getCostOfActions(self, actions):
        ...
        действия включают недопустимый ход, возвращает 999999. 
        """
        if actions == None:
            return 999999
...
def cornersHeuristic(state, problem):
    ...
    "*** ВСТАВЬТЕ ВАШ КОД СЮДА ***"
    pos, visited = state
    not_visited = [corner for corner in corners if corner not in visited]
    result = 0
    while (len(not_visited) != 0):
        distance_to_nearest = float('inf')
        nearest_corner = pos
        for corner in not_visited:
            distance_to_random = util.manhattanDistance(pos, corner)
            if distance_to_nearest > distance_to_random:
                distance_to_nearest, nearest_corner = distance_to_random, corner
        result += distance_to_nearest
        pos = nearest_corner
        not_visited.remove(nearest_corner)
    return result
...
class FoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        self._expanded = 0  # НЕ МЕНЯЙТЕ
        self.heuristicInfo = {}  # Словарь эвристики для хранения информации
...
def foodHeuristic(state, problem):
    """
    Эвристика для FoodSearchProblem,которую вы должны определить.
    ...
    Вы можете обратиться problem. Например, problem.walls вернет вам Grid c   
    расположением стен.
